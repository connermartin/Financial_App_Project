...
print(f"  Latest Close: ${close.iloc[-1].item():.2f}")
print(f"  Period High:  ${close.max().item():.2f}")
print(f"  Period Low:   ${close.min().item():.2f}")
print(f"{'='*50}")
...
current_ma20  = ma20.iloc[-1].item()
current_ma50  = ma50.iloc[-1].item()
...
# Interpret RSI
if current_rsi.item() > 70:
    rsi_signal = "Overbought (Possible SELL Signal)"
    rsi_color  = '#f85149'
elif current_rsi.item() < 30:
...
print(f"{'='*50}")
print(f"  Current RSI   : {current_rsi.item():.2f}")
...
current_vol   = rolling_vol.iloc[-1].item() * 100  # convert to percent
...
# RSI contribution
if current_rsi.item() < 30:
    buy_signals += 2
elif current_rsi.item() > 70:
    sell_signals += 2
